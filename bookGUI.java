...
public class bookGUI{
    ...
    public static void viewBookPage(){
        ...
        JButton bmain1 = new JButton("Back to Main Menu");//creating instance of JButton
        ArrayList<Book> booksInfo = new ArrayList<>();//creating arraylist to store information of books
        ...
        booksInfo = bs.getBooks();//calls getbooks to get all available books in inventory
        
        String[][] books = new String[booksInfo.size()][2];//2d array to store book name and quantity
        String[] columns = new String[] {"Name", "Quantity"};//array to store column names for table
        
        for(int i = 0; i < books.length; i++) {//for loop will fill 2d array with book values from the previous arraylist
        	for(int j = 0; j < books[0].length; j++) {
        		if(j == 0) {
        			books[i][j] = booksInfo.get(i).getName().toString();//fill with book name
        		}
        		else {
        			books[i][j] = String.valueOf(booksInfo.get(i).getQuantity());//fill with book quantity
        		}
        	}
        }
        
        JTable table = new JTable(books, columns);//creating instance of table with book information
        JScrollPane sp = new JScrollPane(table); // adding it to JScrollPane
        
        Border padding = BorderFactory.createEmptyBorder(50, 50, 50, 50);//padding for panel
